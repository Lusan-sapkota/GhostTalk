...
def friends_list_view(request, *args, **kwargs):
    ...
    if user.is_authenticated:
        ...
        return HttpResponse("You must be friends to view their friends list.")
    return JsonResponse({
        'this_user': {'id': context.get('this_user').id, 'username': context.get('this_user').username} if context.get('this_user') else None,
        'friends': [
            {
                'id': f.id,
                'username': f.username,
                'is_mutual': is_mutual
            } for (f, is_mutual) in context.get('friends', [])
        ]
    })
...
def friend_requests(request, *args, **kwargs):
    ...
    return JsonResponse({'friend_requests': [
        ...
        for fr in context.get('friend_requests', [])
    ]})
